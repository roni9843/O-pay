...
export default function SimplePaymentPage() {
  ...
  const [sessionStatus, setSessionStatus] = useState(null); // null | 'expired' | 'already_used' | 'not_found' | 'disabled' | 'paid' | 'ok'
    ...
    async function loadAmount() {
      ...
      try {
        ...
        if (!res.ok || !data.success) {
          ...
          // Classify the error so we can show the right screen
          if (res.status === 410) {
            if (msg.toLowerCase().includes('already been used')) {
              setSessionStatus('already_used');
            } else {
          ...
          setAmountError(msg || 'Invalid or expired payment link');
          setLoadingAmount(false);
          return;
        }

        // Session loaded fine — check if already paid
        if (data.status === 'paid') {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#ececec] px-4">
        <div className="max-w-sm w-full bg-white rounded-3xl shadow-2xl overflow-hidden">
          <div className="h-2 w-full" style={{ background: 'linear-gradient(90deg, #211060, #20CFA2)' }} />
          <div className="p-8 text-center space-y-4">
            <div className="w-16 h-16 mx-auto rounded-2xl flex items-center justify-center text-3xl" style={{ background: 'linear-gradient(135deg,#211060,#20CFA2)' }}>
              🔒
            </div>
            <h2 className="text-lg font-black" style={{ color: '#211060' }}>Invalid Payment Link</h2>
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#ececec] px-4">
        <div className="max-w-sm w-full bg-white rounded-3xl shadow-2xl overflow-hidden">
          ...
          <div className="p-10 text-center space-y-5">
            <div className="w-14 h-14 mx-auto rounded-full border-4 border-indigo-100 border-t-indigo-600 animate-spin" />
            <p className="text-sm font-semibold text-slate-500">Loading payment session…</p>
          </div>
        </div>
      </div>
    );
  ...
  if (sessionStatus === 'expired') {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#ececec] px-4 py-10">
        <div className="max-w-sm w-full bg-white rounded-3xl shadow-2xl overflow-hidden">
          <div className="h-2 w-full bg-rose-500" />
          <div className="p-8 text-center space-y-4">
            <div className="w-16 h-16 mx-auto rounded-2xl flex items-center justify-center text-3xl bg-rose-50 border border-rose-100">
              ⏰
            </div>
            <h2 className="text-lg font-black text-rose-600">Session Expired</h2>
            <p className="text-sm text-slate-500 leading-relaxed">
              এই পেমেন্ট লিংকটির মেয়াদ শেষ হয়ে গেছে। অনুগ্রহ করে মার্চেন্টের সাথে যোগাযোগ করুন এবং নতুন পেমেন্ট লিংক নিন।
            </p>
            <p className="text-xs text-slate-400">This payment session has expired. Please contact the merchant for a new payment link.</p>
            <div className="pt-2 text-xs text-slate-300 font-mono">Session #{code}</div>
          </div>
        </div>
      </div>
    );
  ...
  // Already used / single-use link
  if (sessionStatus === 'already_used') {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#ececec] px-4 py-10">
        <div className="max-w-sm w-full bg-white rounded-3xl shadow-2xl overflow-hidden">
          <div className="h-2 w-full bg-amber-500" />
          <div className="p-8 text-center space-y-4">
            <div className="w-16 h-16 mx-auto rounded-2xl flex items-center justify-center text-3xl bg-amber-50 border border-amber-100">
              🔗
            </div>
            <h2 className="text-lg font-black text-amber-600">Link Already Used</h2>
            <p className="text-sm text-slate-500 leading-relaxed">
              এই পেমেন্ট লিংকটি আগেই ব্যবহার হয়ে গেছে এবং এটি আর বৈধ নয়। মার্চেন্টের সাথে যোগাযোগ করুন।
            </p>
            <p className="text-xs text-slate-400">This is a single-use payment link that has already been accessed. Please request a new link from the merchant.</p>
            <div className="pt-2 text-xs text-slate-300 font-mono">Session #{code}</div>
          </div>
        </div>
      </div>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#ececec] px-4 py-10">
        <div className="max-w-sm w-full bg-white rounded-3xl shadow-2xl overflow-hidden">
          ...
          <div className="p-8 text-center space-y-4">
            <div className="w-16 h-16 mx-auto rounded-2xl flex items-center justify-center text-3xl bg-emerald-50 border border-emerald-100">
              ✅
            </div>
            <h2 className="text-lg font-black text-emerald-600">Already Paid</h2>
            <p className="text-sm text-slate-500 leading-relaxed">
              এই সেশনে পেমেন্ট ইতোমধ্যে সম্পন্ন হয়েছে। ধন্যবাদ!
            </p>
            <p className="text-xs text-slate-400">This payment session has already been completed successfully. Thank you!</p>
            <div className="pt-2 text-xs text-slate-300 font-mono">Session #{code}</div>
          </div>
        </div>
      </div>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#ececec] px-4 py-10">
        <div className="max-w-sm w-full bg-white rounded-3xl shadow-2xl overflow-hidden">
          ...
          <div className="p-8 text-center space-y-4">
            <div className="w-16 h-16 mx-auto rounded-2xl flex items-center justify-center text-3xl bg-slate-50 border border-slate-100">
              🔍
            </div>
            <h2 className="text-lg font-black text-slate-700">Session Not Found</h2>
            <p className="text-sm text-slate-500 leading-relaxed">
              এই পেমেন্ট সেশনটি পাওয়া যাচ্ছে না। লিংকটি সঠিক কিনা নিশ্চিত করুন।
            </p>
            <p className="text-xs text-slate-400">Payment session not found. Please check the link or contact the merchant.</p>
            <div className="pt-2 text-xs text-slate-300 font-mono">Session #{code}</div>
          </div>
        </div>
      </div>
  ...
  // Business disabled or other errors
  if (sessionStatus === 'disabled' || sessionStatus === 'error') {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#ececec] px-4 py-10">
        <div className="max-w-sm w-full bg-white rounded-3xl shadow-2xl overflow-hidden">
          ...
          <div className="p-8 text-center space-y-4">
            ...
            <h2 className="text-lg font-black text-slate-700">Payment Unavailable</h2>
            <p className="text-sm text-slate-500 leading-relaxed">
              {amountError || 'This payment session is currently unavailable. Please try again later or contact the merchant.'}
            </p>
            <div className="pt-2 text-xs text-slate-300 font-mono">Session #{code}</div>
          </div>
        </div>
